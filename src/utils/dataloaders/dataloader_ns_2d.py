...
def split_and_save_h5(raw_h5_loadpath, savepath, dataset_name='NS', 
    ...
    for fname in raw_files:
        ...
        with h5py.File(raw_path, 'r') as f_in:
            vel = f_in['velocity'][:]  # shape: (N, 21, 2, 128, 128)
            file_attrs = dict(f_in.attrs)
            ds_attrs   = dict(f_in['velocity'].attrs)
        ...
        for split, idxs in splits.items():
            ...
            with h5py.File(out_path, 'w') as f_out:
                dset = f_out.create_dataset('velocity', data=vel[idxs])
...
class NS2dDataLoader:
    def __init__(self, data_path, dataset_name='NS'):
    ...
    def load_split(self, split):
        ...
        for fname in files:
            ...
            with h5py.File(path, 'r') as f5:
                ...
                vel = vel.transpose(0,1,3,4,2) # (20000,21,128,128,3)

...
class GSDR2DChunkedIterableDataset(IterableDataset):
    """
    ...
    dataset in memory.
    """
    def __init__(self, data_path, split, ar_order, chunk_size = 5, set_name='GSDR2D',
    ...
    def __iter__(self):
        ...
        for path in paths:
            with h5py.File(path, 'r') as f:
                # load all the fields
                act_A = f['t0_fields/A']    # (160,1001,128,128)
                act_B = f['t0_fields/B']    # (160,1001,128,128)
                
                n_sims = act_A.shape[0]
                for start in range(0, n_sims, self.chunk_size):
                    end = min(start + self.chunk_size, n_sims)
                    A_chunk = act_A[start:end]   # (chunk,1001,128,128)
                    B_chunk = act_B[start:end]   # (chunk,1001,128,128)
                    
                    # expand density to 3 channels
                    A_chunk = np.expand_dims(A_chunk, axis=-1)  # (chunk,1001,128,128,1)
                    B_chunk = np.expand_dims(B_chunk, axis=-1)  # (chunk,1001,128,128,1)
                    
                    # concat A and B
                    batch = np.concatenate((A_chunk, B_chunk), axis=4) # (160,1001,128,128,2)

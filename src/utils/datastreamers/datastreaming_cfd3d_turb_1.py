...
class CFD3DTurbChunkedIterableDataset(IterableDataset):
    def __init__(self, data_path, split, ar_order, chunk_size=5, set_name='CFD3DTurb',
        ...
        for path in paths:
            with h5py.File(path, 'r') as f5:
                ...
                for start in range(0, n_sims, self.chunk_size):
                    ...
                    vx_chunk =  np.expand_dims(vx_chunk, axis = (5,6))                            # (100,21,64,64,64,1,1)
                    vy_chunk =  np.expand_dims(vy_chunk, axis = (5,6))                            # (100,21,64,64,64,1,1)
                    vz_chunk =  np.expand_dims(vz_chunk, axis = (5,6))                            # (100,21,64,64,64,1,1)
                    density_chunk = np.expand_dims(density_chunk, axis = (5,6))                   # (100,21,64,64,64,1,1)
                    pressure_chunk = np.expand_dims(pressure_chunk, axis = (5,6))                 # (100,21,64,64,64,1,1)
                    
                    # make vector fields
                    v_chunk = np.concatenate((vx_chunk, vy_chunk, vz_chunk), axis = 5)                  # (100,21,64,64,64,3,1)
                    density_chunk = np.repeat(density_chunk, repeats = 3, axis = 5)                     # (100,21,64,64,64,3,1)
                    pressure_chunk = np.repeat(pressure_chunk, repeats = 3, axis = 5)                   # (100,21,64,64,64,3,1)

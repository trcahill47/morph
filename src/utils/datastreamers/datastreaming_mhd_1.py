...
class MHDIterableDataset(IterableDataset):
    """
    ...
    dataset in memory.
    """
    def __init__(self, data_path, split, ar_order, chunk_size = 5, set_name='MHD',
    ...
    def _compute_total_samples(self):
        ...
        for p in paths:
            with h5py.File(p, 'r') as f:
                ...
                N, T = f['t1_fields/magnetic_field'].shape[:2]
    ...
    def __iter__(self):
        ...
        for path in paths:
            with h5py.File(path, 'r') as f:
                # load all the fields
                mf_handle = f['t1_fields/magnetic_field']  # HDF5 dataset handle
                vf_handle = f['t1_fields/velocity']
                df_handle = f['t0_fields/density']
                
                n_sims = mf_handle.shape[0]
                for start in range(0, n_sims, self.chunk_size):
                    end = min(start + self.chunk_size, n_sims)
                    mf_chunk = mf_handle[start:end]    # → (chunk_size, T, D, H, W)
                    vf_chunk = vf_handle[start:end]
                    df_chunk = df_handle[start:end]
                    
                    # Expand density to 3 channels: shape -> (T, D, H, W, 3)
                    df_chunk = np.repeat(df_chunk[..., None], 3, axis=-1)
                    
                    # Stack features along last dim: batch shape (n_sims, T, D, H, W, C, 3_fields)
                    batch = np.stack([mf_chunk, vf_chunk, df_chunk], axis=-1)
